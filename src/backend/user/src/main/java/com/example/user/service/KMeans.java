...
import com.example.user.entity.Point;
...
public class KMeans {
    ...
    private List<Long> user_id_list =null;    //用于存放，原始数据集
    ...
    public KMeans(int k, List<float[]> original_data, List<Long> user_id_list) {
        this.kNum = k;
        this.original_data = original_data;
        this.len = original_data.get(0).length;
    ...
    private void init() {
        ...
        for (int i = 0, j = original_data.size(); i < j; i++){
            pointList.add(new Point(i, original_data.get(i), user_id_list.get(i)));
        }
    }
